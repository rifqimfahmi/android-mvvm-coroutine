...
class MatchesAdapter(var resource: Resource<List<Match>>, var clickListener: (Match) -> Unit)
    ...
    fun submitData(data: Resource<List<Match>>?) {
        data?.let {
            resource = it
            ...
            match?.let { evt ->
                with (itemView) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        itemView.stateListAnimator = AnimatorInflater.loadStateListAnimator(this.context, R.animator.lift_on_touch)
                    }
                    setOnClickListener {
                        clickListener(evt)
                    }
